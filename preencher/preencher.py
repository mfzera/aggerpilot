# Arquivo: preencher/preencher.py (VERSÃO COM A CHAMADA CORRETA)
...
def executar_preenchimento(dados_cliente: dict) -> bool:
    """
    Orquestra o preenchimento completo do formulário.
    """
    try:
        ...
        # Passa o dicionário 'dados_cliente' completo para a função.
        ...
        nomes_dos_arquivos_para_anexar = [os.path.basename(caminho) for caminho in dados_cliente.get("anexos", [])]
        if not nomes_dos_arquivos_para_anexar:
            print("⚠️  Nenhum anexo encontrado para este cliente. Pulando etapa.")
        elif not anexar_arquivos(nomes_dos_arquivos_para_anexar):
            print("🚨 Falha na Etapa 5. Abortando preenchimento.")
            return False
        print("--- Etapa 5 concluída com sucesso! ---")

        print("\n--- Etapa 6: Salvando a prospecção ---")
        if not salvar_prospeccao():
            print("🚨 Falha na Etapa 6. Abortando preenchimento.")
            return False
