# Arquivo: main.py (VERSÃO FINAL COM CONVERSÃO DE IMAGEM)
...
# --- CORREÇÃO: Importa 'Tuple' com 'T' maiúsculo ---
...
# --- CORREÇÃO: A sintaxe do tipo de retorno foi atualizada para usar 'Tuple' ---
def preparar_anexos_do_cliente(arquivos_originais: list) -> Tuple[list, list]:
    """
    ...
    for caminho_arquivo in arquivos_originais:
        ...
        if extensao.lower() in ['.jpg', '.jpeg', '.png', '.bmp', '.gif']:
            ...
            try:
                ...
                caminho_pdf_convertido = nome_base + ".pdf"
...
def processar_propostas_locais():
    ...
    for caminho_arquivo in lista_de_arquivos:
        ...
        nome_base_cliente = re.sub(r'\d+$', '', nome_base).strip()
    ...
    for nome_base_cliente in clientes_unicos:
        ...
        try:
            ...
            arquivos_originais_do_cliente = glob.glob(os.path.join(CAMINHO_LOCAL_PROPOSTAS, f"{nome_base_cliente}*.*"))
            pdfs_para_anexar, imagens_para_limpar = preparar_anexos_do_cliente(arquivos_originais_do_cliente)
                ...
                for arquivo_local in arquivos_a_mover:
                    ...
                    try:
                        ...
                        with GerenciadorSFTP(SSH_CONFIG) as sftp:
                            # Encontra o arquivo original na VPS (pode ser .pdf, .jpg, etc.)
                            arquivos_remotos = sftp.listar_arquivos(CAMINHO_REMOTO_PDFS)
                            nome_base_remoto = os.path.splitext(nome_do_arquivo)[0]
